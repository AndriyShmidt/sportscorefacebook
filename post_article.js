...
async function postOnInstagram(item, match) {
  ...
  try {
    instagramResponse = await fetch(`https://graph.facebook.com/v18.0/17841462745627692/media?image_url=http://localhost:3000${myConvertedImagePath}&caption=${encodeURIComponent(instagramMessage)}&access_token=${userToken}`, {
      method: 'POST',
    });
  } catch (error) {
